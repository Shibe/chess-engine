...
int is_mate(int player, Pieces *player_pieces, Pieces *opponent_pieces, Bitboard player_attacks, Bitboard opponent_attacks, Chessboard *chessboard) {
	...
	Bitboard check_lines = get_attacks_on_king(player_pieces->king, opponent_pieces, player);
	...
	for (Bitboard a = 1ULL; a; a = a << 1) {
		...
		if (player_pieces->king & a) {
			...
			if (intercepts && (!(intercepts&(intercepts-1)))) { // If the amount of intecepts is power of 2 (only 1 bit is set to 1)
				Bitboard copy = player_pieces->king;
				player_pieces->king = (player_pieces->king & ~a) | intercepts;

				Pieces opponent_copy = *opponent_pieces;
				move_piece(&opponent_copy, intercepts, 0x0ULL);
				update_pieces(&opponent_pieces);
				update_chessboard(chessboard);
				Bitboard checked = is_checked(player_pieces->king, compute_attacking_squares(!player, &opponent_copy, player_pieces, chessboard->en_passant_target));
				player_pieces->king = copy;
				update_chessboard(chessboard);
				if (!checked) {
					return 0;	
				}
...
Bitboard is_double_check(Bitboard king, Bitboard attacks_on_king) {
    Bitboard king_mask = compute_king(king, 0x0ULL, clear_file);
...
Bitboard get_attacks_on_king(Bitboard king, Pieces *opponent_pieces, int player) {
    Bitboard horizontal_check_lines = compute_straight_check_lines(king, opponent_pieces, clear_file);
	Bitboard diagonal_check_lines = compute_diagonal_check_lines(king, opponent_pieces, clear_file);
...
Bitboard compute_straight_check_lines(Bitboard king, Pieces *opponent_pieces, Bitboard clear_file[FILE_LEN]) {
    ...
    Bitboard p;
	for (p = king << 1; p; p |= p << 1) {
        if (p & straight_sliders) {
            check_lines |= p;
            break;
        } else if (p & ~clear_file[FILE_H]) {
			break;
		}
	}
    ...
    for (p = king >> 1; p; p |= p >> 1) {
        if (p & straight_sliders) {
            check_lines |= p;
            break;
        } else if (p & ~clear_file[FILE_A]) {
			break;
		}
	}

	for (p = king << 8; p; p |= p << 8) {
        if (p & straight_sliders) {
            check_lines |= p;
            break;
        }
...
Bitboard compute_attacking_squares(int player, Pieces *own_side, Pieces *opposing_side, Bitboard en_passant_target) { //TODO: Also include own pieces as possible points of attack.
